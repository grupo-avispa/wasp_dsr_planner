...
#include "wasp_dsr_planner/executor/action/get_role.hpp"

GetRole::GetRole(
  const std::string & xml_tag_name, const std::string & action_name,
  const BT::NodeConfiguration & conf)
: DSRAction<get_role_node_type>(xml_tag_name, action_name, conf)
{
}

void GetRole::getInputsOnTick()
{
  getInput<std::string>("question", question_);
}

bool GetRole::setAttributesBeforeStart(DSR::Node & node)
{
  bool success = false;
  if (!question_.empty()) {
...
bool GetRole::setOutputsAfterFinished(DSR::Node & node)
{
  bool success = false;
...
BT_REGISTER_NODES(factory) {
  BT::NodeBuilder builder =
    [](const std::string & name, const BT::NodeConfiguration & config) {
      return std::make_unique<GetRole>(name, "get_role", config);
    };

  factory.registerBuilder<GetRole>("GetRole", builder);
}
