...
#include "wasp_dsr_planner/executor/action/get_task.hpp"

GetTask::GetTask(
  const std::string & xml_tag_name, const std::string & action_name,
  const BT::NodeConfiguration & conf)
: DSRAction<get_task_node_type>(xml_tag_name, action_name, conf)
{
}

void GetTask::getInputsOnTick()
{
  getInput<std::string>("question", question_);
}

bool GetTask::setAttributesBeforeStart(DSR::Node & node)
{
  bool success = false;
  if (!question_.empty()) {
...
bool GetTask::setOutputsAfterFinished(DSR::Node & node)
{
  bool success = false;
...
BT_REGISTER_NODES(factory) {
  BT::NodeBuilder builder =
    [](const std::string & name, const BT::NodeConfiguration & config) {
      return std::make_unique<GetTask>(name, "get_task", config);
    };

  factory.registerBuilder<GetTask>("GetTask", builder);
}
